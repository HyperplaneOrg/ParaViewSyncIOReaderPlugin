...
void vtkPhastaSyncIOReader::ReadGeomFile(char* geomFileName,
		int &firstVertexNo,
		vtkPoints *points,
		int &num_nodes,
		int &num_cells,
        ...
        int **evm1, int **evm2)
{
  ...
  vtkDebugMacro("In P ReadFieldFile (vtkUnstructuredGrid): fieldFileName="<<fieldFileName << ", partID_counter=" << partID_counter);
	...
	vtkPhastaSyncIOReaderInternal::FieldInfoMapType::iterator it = this->Internal->FieldInfoMap.begin();
	vtkPhastaSyncIOReaderInternal::FieldInfoMapType::iterator itend = this->Internal->FieldInfoMap.end();
	for(; it!=itend; it++)
	{
...
void vtkPhastaSyncIOReader::PrintSelf(ostream& os, vtkIndent indent)
{
